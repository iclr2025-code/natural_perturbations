...
results = extract_information('../data.csv') # this is the csv file containing the extracted candidate original and perturbed passage pairs for squad 1.1
...
original = Load_Read_SQuAD_Json_File('../SQuAD/dev-v1.1.json')

corresponding = [original[1][index] for index in [original[0].index(title) for title in results[0]]]

common_items = []

for id in range(len(corresponding)):
  common_items.append(list(set(corresponding[id]) & set(results[1][id])))

not_match = [index for index in range(len(common_items)) if len(common_items[index]) == 0]

titles_pro = [results[0][index] for index in range(len(results[0])) if index not in not_match]
...
SQuAD_First_Version_Dev_Set = Load_SQuAD_First_Version_Dev_Set('../SQuAD/dev-v1.1.json')
...
with open('./dev-v1.1-original-pre.json', 'w') as f:
  json.dump(SQuAD_First_Version_Dev_Set, f)

modified_context_pro = [results[2][index] for index in range(len(results[2])) if index not in not_match]
reference = [results[1][index] for index in range(len(results[1])) if index not in not_match]
...
for findex in range(len(contexts_pro)):
  ...
  temp = []
  for sub in common:
    temp.append(reference[findex].index(sub))
  for t in temp:
    modified_contexts_keep.append(modified_context_pro[findex][t])
...
SQuAD_First_Version_Dev_Set = Load_json_data('./dev-v1.1-original-pre.json')

for title_id in tqdm.tqdm(range(len(SQuAD_First_Version_Dev_Set['data']))):
  paragraphs_squad_first_version_dev_set = SQuAD_First_Version_Dev_Set['data'][title_id]['paragraphs']
  for paragraph_squad_first_version_dev_set in paragraphs_squad_first_version_dev_set:
    context_squad_first_version_dev_set = paragraph_squad_first_version_dev_set['context']
    context_index = contexts_keep.index(context_squad_first_version_dev_set)
    paragraph_squad_first_version_dev_set['context'] = modified_contexts_keep[context_index]

    qass_squad_first_version_dev_set = paragraph_squad_first_version_dev_set['qas']
    for qas_squad_first_version_dev_set in qass_squad_first_version_dev_set:
      question_squad_first_version_dev_set = qas_squad_first_version_dev_set['question']

      answers_squad_first_version_dev_set = qas_squad_first_version_dev_set['answers']
      for answer_squad_first_version_dev_set in answers_squad_first_version_dev_set:
        ground_truth_answer_squad_first_version_dev_set = answer_squad_first_version_dev_set['text']
        answer_squad_first_version_dev_set['answer_start'] = find_continuous_span_start(modified_contexts_keep[context_index], ground_truth_answer_squad_first_version_dev_set)

with open('./dev-v1.1-np-pre.json', 'w') as f:
  json.dump(SQuAD_First_Version_Dev_Set, f)

d1 = Load_json_data('./dev-v1.1-np-pre.json')
...
d2 = Load_json_data('./dev-v1.1-original-pre.json')
...
d4 = Load_json_data('./dev-v1.1-original-pre.json')
...
with open('./dev-v1.1-original-new.json', 'w') as f:
  json.dump(d4, f)

d5 = Load_json_data('./dev-v1.1-original-new.json')
...
with open('./dev-v1.1-np-new.json', 'w') as f:
  json.dump(d5, f)